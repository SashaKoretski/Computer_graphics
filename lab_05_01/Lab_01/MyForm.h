...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->label_title->Text = L"Лабораторная по компьютерной графике №5";
			// 
			// btn_collapce
			// 
			...
			this->button_color->Font = (gcnew System::Drawing::Font(L"Trebuchet MS", 9.75F, System::Drawing::FontStyle::Regular, System::Drawing::GraphicsUnit::Point,
				static_cast<System::Byte>(204)));
			this->button_color->Location = System::Drawing::Point(992, 539);
			...
			this->button_clear->Font = (gcnew System::Drawing::Font(L"Trebuchet MS", 9.75F, System::Drawing::FontStyle::Regular, System::Drawing::GraphicsUnit::Point,
				static_cast<System::Byte>(204)));
			this->button_clear->Location = System::Drawing::Point(992, 498);
...
#pragma endregion
		...
		private: System::Void условиеЗадачиToolStripMenuItem_Click(System::Object^ sender, System::EventArgs^ e) {
			MessageBox::Show("Фигура задается координатами ее точек. Необходимо реализовать алгоритм"
				"закрашивания фигуры с задержкой и без.",
				"Условие задачи №5");
		}
		...
		//реверсируем цвет пикселя
		private: void reverse_pixel(Graphics^ g, int x, int y) {
			SolidBrush^ brush = gcnew SolidBrush(MyColor);
			SolidBrush^ brush_white = gcnew SolidBrush(Color::White);

			myPoint^ p = gcnew myPoint(x, y);

			for (int i = 0; i < coloredPix->Count; i++) {
				if (coloredPix[i] == p) {
					coloredPix->RemoveAt(i);
					g->FillRectangle(brush_white, x, y, 1, 1);
					return;
				}
			}
		...
		// проверка точки, является ли она локальным максимумом или минимумом 
		private: bool check_point_maxmin(int i) {
			double y_now = points[i]->Y;
			int j = i - 1;
			int k = i + 1;
			while (j >= 0 && points[j]->Y == y_now)
				j--;
			while (k < points->Count && points[k]->Y == y_now)
				k++;

			if (k >= points->Count || i < 0)
				return false;
			if ((points[j]->Y < y_now && y_now > points[k]->Y) || (points[j]->Y > y_now && y_now < points[k]->Y))
				return true;
			return false;
		}
		...
		private: void wait_color() {
			Graphics^ g = surface->CreateGraphics();
			double septum = points[0]->X;
			SolidBrush^ brush = gcnew SolidBrush(MyColor);

			for (int i = 0; i < points->Count; i++) {
				if (i > 0 && points[i]->Y != points[i - 1]->Y) {
					if (!check_point_maxmin(i)) {
						if (points[i]->X < septum) {
							for (int j = points[i]->X + 1; j <= septum; j++) {
								reverse_pixel(g, j, points[i]->Y);
							}
						}
						else if (points[i]->X > septum) {
							for (int j = points[i]->X; j > septum; j--) {
								reverse_pixel(g, j, points[i]->Y);
							}
						}
					}
				}
				g->FillRectangle(brush, points[i]->X, points[i]->Y, 1, 1);
			}
		}

		//реверсируем цвет пикселя без закраски
		private: void reverse_pixel_no_draw(Graphics^ g, int x, int y) {
			...
			for (int i = 0; i < points->Count; i++) {
				if (i > 0 && points[i]->Y != points[i - 1]->Y) {
					if (!check_point_maxmin(i)) {
						if (points[i]->X < septum) {
							for (int j = points[i]->X + 1; j <= septum; j++) {
								reverse_pixel_no_draw(g, j, points[i]->Y);
							}
						}
						else if (points[i]->X > septum) {
							for (int j = points[i]->X; j > septum; j--) {
